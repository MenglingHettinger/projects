#' Random Forest Classifier using the randomForest Package with ordinal encoding and missing value imputation
#'
...
modelfit <- function(response, data, extras) {
  ...
  response <- factor(as.character(response), levels=c("0", "1"))
  ...
  if (length(mnum_cols) > 0) {
    ...
    if(length(single_val) > 0) {
        for(i in 1:length(single_val)){      val <- max(data[mnum_cols[single_val[i]]], na.rm=TRUE)
  ...
  # Create data matrix for fitting
  data <- data.matrix(data[c(mchar_cols_low_card, mchar_cols_high_card, mnum_cols, mvcols)])
  ...
  # Fit the model
  MTRY <- min(floor(sqrt(ncol(data))), 50)
  mrf <- randomForest(x=data, y=response, ntree=NTREES, nodesize=NODESIZE, mtry=MTRY, replace=FALSE)
...
modelpredict <- function(model, data) {
  ...
  if (length(model$char_cols) > 0) {
    ...
    # And ordinal encode
    data[model$char_cols_low_card] <- lapply(data[model$char_cols_low_card], FUN=as.integer)
  ...
  data <- data.matrix(data[c(model$char_cols_low_card, model$char_cols_high_card, model$num_cols, model$mvcols)])  
  
  # Return predictions
  predictions <- predict(model[['rf']], newdata=data, type='prob')[,2]
  
};

datarobot.run("R Random Forest Classifier Reference Model", modelfit, modelpredict)
